...
export function playGiantSquidGame(input: Input): number {
  ...
  for (let index = 0; index < input.numbersToDraw.length; index++) {
    ...
    for (let i = 0; i < input.boards.length; i++) {
      ...
      const winner: Winner = checkForWinner(input.boards);

      if (winner.winnerFound) {
        winnerFound = winner.winnerFound;

        unfoundNumbers = winner.unFoundNumbers;
...
export function checkForWinner(boards: Array<Array<Array<BoardItem>>>): Winner {
  let unfoundNumbers: Array<BoardItem> = [];
  let winnerFound = false;
  ...
  for (let index = 0; index < boards.length; index++) {
    ...
    for (let i = 0; i < board.length; i++) {
      ...
      if (winnerFound) {
        const winnerBoard: Array<Array<BoardItem>> = board.splice(5, 5);
